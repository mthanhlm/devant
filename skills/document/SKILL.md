...
description: devant specialist (work-invoked): write or update docs grounded in the devant graph (code + intent)'s vision/audience. Updates in place; no stray files.
---
...
- **Verify every claim before you ship it** — run or cross-check each command, code example, and
  ...
  rather than asserting it (the grounding rule). A stale or wrong example is worse than none.
