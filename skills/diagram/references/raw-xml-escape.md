# raw-XML escape hatch — style system & skeletons (guide §1–§5, §7)

Read ONLY on the escape path: a shape the diagram spec can't express (swimlanes, fork/join bars,
C4 boundary boxes), hand-authored as mxGraph XML per the sections below, laid out with
`devant layout <file> --preset <p>`, then linted clean (`devant drawio-lint --fix`). Everything
here inherits the altitude rules and honesty bar of `drawio-guide.md` §0.
...
Confirm it actually reads on the on-demand `drawio-preview` visual pass (SKILL step 6), not by the px number.
...
(SKILL step 5). Hand-routed edges (explicit `<Array as="points">` waypoints) additionally get
...
- `devant diagram-build` exited 0 (spec path) — or, on the raw path, `devant layout` has placed
  ...
  label-collision gate (SKILL step 5).
